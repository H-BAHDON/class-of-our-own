...
async function handleSignpost(req, res) {
  try {
    ...
    const [numOfRows, updatedRows] = await User.update(
      { traineeCodwarsUsername: codeWarsUsername, traineeCodilityUsername: codilityUsername },
      { where: { email: req.user.email } }
    );

    if (numOfRows === 0) {
      return res.status(404).json({ error: "User not found for the authenticated user." });
    }

    const updatedUser = await User.findOne({ where: { email: req.user.email } });
    return res.status(200).json({ message: "Usernames saved successfully.", user: updatedUser });
  } catch (err) {
